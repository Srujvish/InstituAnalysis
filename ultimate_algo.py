...
# --------- LOT SIZES AND PROFIT CAPS ---------
LOT_CONFIG = {
    "NIFTY": {
        "lots": 2,
        "quantity_per_lot": 75,  # 2 lots = 150 total
        "profit_cap": 10,  # Max profit per share
        "loss_multiplier": 1.0  # Full loss calculation
    },
    "BANKNIFTY": {
        "lots": 2,
        "quantity_per_lot": 35,  # 2 lots = 70 total
        "profit_cap": 12,  # Max profit per share
        "loss_multiplier": 1.0
    },
    "SENSEX": {
        "lots": 2,
        "quantity_per_lot": 20,  # 2 lots = 40 total
        "profit_cap": 15,  # Max profit per share
        "loss_multiplier": 1.0
    },
    "MIDCPNIFTY": {
        "lots": 1,
        "quantity_per_lot": 140,  # 1 lot = 140 total
        "profit_cap": 8,  # Max profit per share
        "loss_multiplier": 1.0
    }
}
...
def is_market_open():
    ...
    return dtime(9,15) <= current_time_ist <= dtime(15,20)  # Changed to 3:20 PM

def should_stop_trading():
    utc_now = datetime.utcnow()
    ist_now = utc_now + timedelta(hours=5, minutes=30)
    current_time_ist = ist_now.time()
    return current_time_ist >= dtime(15,20)  # Changed to 3:20 PM
...
def calculate_pnl(entry, max_price, min_price, targets, targets_hit, sl):
    try:
        if targets is None or len(targets) == 0:
            # Use max_price for profit, min_price for loss calculation
            if max_price > entry:
                profit_move = max_price - entry
                return f"+{profit_move:.2f}"
            elif min_price < entry:
                loss_move = entry - min_price
                return f"-{loss_move:.2f}"
            else:
                return "0"
        ...
        if achieved_prices:
            ...
            # Use min_price for loss calculation if price went below entry
            if min_price < entry:
                loss_move = entry - min_price
                return f"-{loss_move:.2f}"
            elif max_price > entry:
                profit_move = max_price - entry
                return f"+{profit_move:.2f}"
            else:
                return "0"
...
def monitor_price_live(symbol, entry, targets, sl, fakeout, thread_id, strategy_name, signal_data):
    def monitoring_thread():
        ...
        min_price_reached = entry  # Track minimum price for loss calculation
        ...
        while True:
            ...
            if should_stop_trading():
                try:
                    final_pnl = calculate_pnl(entry, max_price_reached, min_price_reached, targets, targets_hit, sl)
                except Exception:
                    final_pnl = "0"
                signal_data.update({
                    ...
                    "min_price_reached": min_price_reached,  # Store min price for loss calculation
            ...
            if price:
                ...
                # Track maximum price (for profit calculation)
                if price > max_price_reached:
                    max_price_reached = price
                
                # SILENTLY track minimum price (for loss calculation) - no Telegram messages
                if price < min_price_reached:
                    min_price_reached = price
                ...
                if not in_trade:
                    ...
                    # Only send Telegram messages for price increases (new highs)
                    if price > last_high:
                    ...
                    if price <= sl:
                        ...
                        signal_data.update({
                            "targets_hit": sum(targets_hit),
                            "max_price_reached": max_price_reached,
                            "min_price_reached": min_price_reached,
                            "zero_targets": sum(targets_hit) == 0,
                            "no_new_highs": max_price_reached <= entry,
                            "final_pnl": final_pnl
                        })
...
# 🚨 NEW: INVESTMENT CALCULATIONS WITH PROFIT CAPS 🚨
def calculate_investment_pnl(signal_data):
    """Calculate investment P&L with profit caps per index"""
    index = signal_data.get('index')
    entry_price = signal_data.get('entry_price')
    max_price_reached = signal_data.get('max_price_reached', entry_price)
    min_price_reached = signal_data.get('min_price_reached', entry_price)  # Use min price for loss
    sl = signal_data.get('sl')
    entry_status = signal_data.get('entry_status', 'PENDING')
    
    if index not in LOT_CONFIG or entry_price is None:
        return {
            'investment': 0,
            'profit': 0,
            'loss': 0,
    ...
    config = LOT_CONFIG[index]
    total_quantity = config['lots'] * config['quantity_per_lot']
    profit_cap = config['profit_cap']
    
    investment = entry_price * total_quantity
    ...
    # Calculate price movement for profit (using max_price_reached)
    price_move_profit = max_price_reached - entry_price
    
    # Calculate price movement for loss (using min_price_reached)
    price_move_loss = entry_price - min_price_reached
    
    # Apply profit cap - if price goes above cap, use cap value
    if price_move_profit > profit_cap:
        profit = profit_cap * total_quantity
    # If price didn't reach profit cap but went up
    elif price_move_profit > 0:
        profit = price_move_profit * total_quantity
    # If price didn't go up
    else:
        profit = 0
    
    # Calculate loss based on minimum price reached below entry
    if price_move_loss > 0:
        loss = price_move_loss * total_quantity
    else:
        loss = 0
...
# 🚨 FIXED: WORKING EOD REPORT SYSTEM WITH INVESTMENT CALCULATIONS 🚨
def send_individual_signal_reports():
    ...
    for i, signal in enumerate(unique_signals, 1):
        ...
        investment_data = calculate_investment_pnl(signal)
        
        # Add to totals
        total_investment += investment_data['investment']
        total_profit += investment_data['profit']
        total_loss += investment_data['loss']
        total_net_pnl += investment_data['net_pnl']
        
        index_config = LOT_CONFIG.get(signal.get('index', ''), {})
        lots = index_config.get('lots', 0)
        quantity_per_lot = index_config.get('quantity_per_lot', 0)
        total_quantity = lots * quantity_per_lot
        profit_cap = index_config.get('profit_cap', 0)
        ...
        msg += (f"• Max Price Reached: ₹{signal.get('max_price_reached', signal.get('entry_price','?'))}\n"
                f"• Min Price Reached: ₹{signal.get('min_price_reached', signal.get('entry_price','?'))}\n"
                f"• Final P&L: {signal.get('final_pnl', '0')} points\n\n"
                
                f"💼 INVESTMENT CALCULATIONS:\n"
                f"• Lots: {lots} x {quantity_per_lot} = {total_quantity} quantity\n"
                f"• Investment: ₹{signal.get('entry_price','?')} x {total_quantity} = ₹{investment_data['investment']:,.0f}\n"
                f"• Profit Cap: ₹{profit_cap} per share\n")
        
        if investment_data['profit'] > 0:
            msg += f"• Profit: ₹{investment_data['profit']:,.0f}\n"
        if investment_data['loss'] > 0:
            msg += f"• Loss: ₹{investment_data['loss']:,.0f}\n"
        
        msg += (f"• Net P&L: ₹{investment_data['net_pnl']:+,.0f}\n\n"
    ...
    # Send summary with investment totals
    summary_msg = (f"📈 DAY SUMMARY\n"
                   ...
                   f"• Net P&L: ₹{total_net_pnl:+,.0f}\n"
                   f"• ROI: {(total_net_pnl/total_investment*100 if total_investment > 0 else 0):+.1f}%\n"
                   f"─────────────────────────────")
...
def send_signal(index, side, df, fakeout, strategy_key):
    ...
    signal_data = {
        ...
        "targets_hit": 0,
        "max_price_reached": entry,
        "min_price_reached": entry,  # Initialize min price same as entry
        "zero_targets": True,
        "no_new_highs": True,
        "final_pnl": "0"
    }
...
def run_algo_parallel():
    ...
    if should_stop_trading():
        ...
        if not STOP_SENT:
            send_telegram("🛑 Market closed at 3:20 PM IST - Algorithm stopped")
            STOP_SENT = True
...
while True:
    try:
        ...
        if not market_open:
            ...
            # 🚨 COMPULSORY EOD REPORT TRIGGER BETWEEN 3:20 PM - 4:00 PM
            if current_time_ist >= dtime(15,20) and current_time_ist <= dtime(16,0) and not EOD_REPORT_SENT:
                send_telegram("📊 GENERATING COMPULSORY END-OF-DAY REPORT...")
                time.sleep(10)
                send_individual_signal_reports()
                EOD_REPORT_SENT = True
        ...
        if not STARTED_SENT:
            send_telegram("🚀 GIT ULTIMATE MASTER ALGO STARTED - 4 Indices Running\n"
                         ...
                         "✅ Guaranteed EOD Reports at 3:20 PM\n"
